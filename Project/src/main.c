...
pthread_mutex_t mtx_incr_found;
pthread_mutex_t mtx_flag_found;
pthread_cond_t cnd_pass_found; 

/*Make a function that will init global itself: set passwd_found to 0, set filenames, etc.*/
...
void* show_results(){
    ...
    while (globalData.users_len != globalData.passwd_found){ //if not all password have been found.
        int idx = globalData.passwd_found-1;
        printf("HERE\n");
...
int main(int argc, char* argv[]){
    ...
    pthread_mutex_init(&mtx_incr_found, NULL);
    ...
    pthread_mutex_destroy(&mtx_incr_found);
